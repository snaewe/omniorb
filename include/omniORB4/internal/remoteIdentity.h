...
public:
  inline omniRemoteIdentity(omniIOR* ior, _OMNI_NS(Rope)* rope)
    : omniIdentity((const CORBA::Octet*)
		   ior->getIIOPprofile().object_key.get_buffer(),
		   ior->getIIOPprofile().object_key.length()),
      pd_refCount(0),
      pd_ior(ior),
      pd_rope(rope)
    {}
