...
\dfunc{pragmas()}
  List of strings containing \texttt{\#pragma}s which occurred before
  any declarations. Later \texttt{\#pragma}s are attached to
  \class{Decl} objects.

\dfunc{comments()}
  List of strings containing comments which were not attached to
  declarations (see section~\ref{sec:comments}).
...
\dfunc{pragmas()}
  List of strings containing \texttt{\#pragma}s which occurred after
  this declaration, but before any others.

\dfunc{comments()}
  List of strings containing comments attached to this declaration
