...
static char* internal_make_include_filename(const char* bname);
static char* internal_escape_non_alnum_chars(const char* s);

const char* defs_fragment_suffix      = "_defs";
const char* operators_fragment_suffix = "_operators";
const char* poa_fragment_suffix       = "_poa";


// Do not forget to update the version number in omniORB_x_y below.
// The variable is defined in omniInternal.h.
#define OMNIORB_LIBRARY_VERSION    "omniORB_3_0"
...
o2be_root::produce()
{
  ...
  try {
    ...
    baselen    = ep ? (ep - bname) : strlen(bname);
    ...
    if (!(idl_global->compile_flags() & IDL_BE_GENERATE_FRAGMENT)) {
      ...
      stubfname.clear();
      stubfname += basename;
      stubfname += o2be_global::hdrsuffix();
	...
	throw o2be_fileio_error("Can't open output header file");
      ...
      stubfname.clear();
      stubfname += basename;
      stubfname += defs_fragment_suffix;
      stubfname += o2be_global::hdrsuffix();
	...
	throw o2be_fileio_error("Can't open output header file");
      ...
      stubfname.clear();
      stubfname += basename;
      stubfname += operators_fragment_suffix;
      stubfname += o2be_global::hdrsuffix();
	...
	throw o2be_fileio_error("Can't open output header file");
      ...
      stubfname.clear();
      stubfname += basename;
      stubfname += poa_fragment_suffix;
      stubfname += o2be_global::hdrsuffix();
      hdr_poa.open(stubfname, std::ios::out | std::ios::trunc);
      if (! hdr_poa)
	throw o2be_fileio_error("Can't open output header file");
    }
    ...
    stubfname.clear();
    stubfname += basename;
    stubfname += o2be_global::skelsuffix();
    ...
    if (idl_global->compile_flags() & IDL_CF_ANY) {
      ...
      stubfname.clear();
      stubfname += basename;
      stubfname += o2be_global::dynskelsuffix();
	...
	throw o2be_fe_error("Error detected when generating the header file.");
      ...
      hdr_defs << "// This file is generated by omniidl3- "
	...
	throw o2be_fe_error("Error detected when generating the header file.");
      ...
      hdr_opers << "// This file is generated by omniidl3- "
		<< (const char*) OMNIORB_LIBRARY_VERSION << ". Do not edit.\n";
	...
	throw o2be_fe_error("Error detected when generating the header file.");
      ...
      hdr_poa << "// This file is generated by omniidl3- "
		<< (const char*) OMNIORB_LIBRARY_VERSION << ". Do not edit.\n";
      hdr_poa << "#ifndef __" << basename << poa_fragment_suffix << "_hh__\n"
	      << "#define __" << basename << poa_fragment_suffix << "_hh__\n\n";
      produce_poa_hdr(hdr_poa);
      hdr_poa << "#endif" << std::endl;
      if (idl_global->err_count() > 0)
	throw o2be_fe_error("Error detected when generating the header file.");
      hdr_poa.close();
    }
...
{
  StringBuf escaped_basename;
  escaped_basename.grab(internal_escape_non_alnum_chars(basename));

  s << "// This file is generated by omniidl3- "
    << (const char*) OMNIORB_LIBRARY_VERSION << ". Do not edit.\n";
  s << "#ifndef __" << escaped_basename.c_str() << "_hh__\n"
    << "#define __" << escaped_basename.c_str() << "_hh__\n\n"
...

  {
	...
	continue;
      ...
      StringBuf filename, guardname;
      filename.grab(internal_make_include_filename(bname));
      guardname.grab(internal_escape_non_alnum_chars(filename));

      s << "#ifndef __" << guardname.c_str() << "_EXTERNAL_GUARD__\n"
	<< "#define __" << guardname.c_str() << "_EXTERNAL_GUARD__\n"
  ...
  produce_hdr_defs(s);
  o2be_module::produce_poa_hdr(s);
  if( idl_global->compile_flags() & IDL_BE_GENERATE_FLAT_TIE )
  ...
  s << "#endif  // __" << escaped_basename.c_str() << "_hh__" << std::endl;
}
...
o2be_root::produce_hdr_operators(std::fstream &s)
{
  ...
  o2be_module::produce_defns_at_global_scope_in_hdr(s);
}
...
o2be_root::produce_skel(std::fstream &s)
{
  ...
  if (!(idl_global->compile_flags() & IDL_BE_GENERATE_FRAGMENT)) {
    ...
    s << "#include <omniORB3/callDescriptor.h>\n\n\n";
  ...
  o2be_module::produce_poa_skel(s);
}
...
o2be_root::produce_dynskel(std::fstream& s)
{
  ...
  if (!(idl_global->compile_flags() & IDL_BE_GENERATE_FRAGMENT)) {
    ...
    s << "#include \"" << basename << o2be_global::hdrsuffix() << "\"\n";
    s << "#include <omniORB3/tcDescriptor.h>\n\n";
...
IMPL_NARROW_METHODS2(o2be_root, AST_Root, o2be_module)
...
static char*
internal_escape_non_alnum_chars(const char* s)
{
