...
////////////////////////////////////////////////////////////////////////
IOP_C*
giopRope::acquireClient(const omniIOR* ior,omniCallDescriptor* calldesc) {

  GIOP::Version v = ior->getIIOPprofile().version;
...
giopRope::selectRope(const giopAddressList& addrlist,
  ...
  last = addrlist.end();
  for (; i != last; i++) {
    if (omniObjAdapter::matchMyEndpoints(*i)) {
      r = 0; loc = 1;
      return 1;
    }
  }
