...
/*
 $Log$
 Revision 1.2  2000/07/04 15:22:55  dpg1
...
public:
  ...
  virtual void dispatch(GIOP_S&, omniLocalIdentity*) = 0;
  // Dispatch request to given object.

  virtual void dispatch(GIOP_S&, const _CORBA_Octet* key, int keysize) = 0;
  ...
  static ropeFactoryList* incomingRopeFactories();

  static Rope* defaultLoopBack();
  // Does not increment the reference count.

  struct ListenPort {
    inline ListenPort(const char* h, unsigned p) : host(h), port(p) {}
    inline ListenPort() : port(0) {}
    CORBA::String_var host;
    unsigned          port;
  };
  typedef omnivector<ListenPort> ListenPortList;

  struct Options {
    inline Options() : noBootstrapAgent(0) {}

    _CORBA_Boolean noBootstrapAgent;
    ListenPortList incomingPorts;
  };
